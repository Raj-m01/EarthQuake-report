...
public final class QueryUtils {
    ...
    public static ArrayList<info> extractEarthquakes() {
        ...
        try {
            ...
            JSONObject jroot = new JSONObject(SAMPLE_JSON_RESPONSE);

           JSONArray jsonArray = jroot.getJSONArray("features");
           ...
           for(int i=0;i<jsonArray.length();i++){
               ...
               JSONObject arr = firstObj.getJSONObject("properties");
                location = arr.getString("place");
                mag = arr.getDouble("mag");
                time = arr.getLong("time");
                details = arr.getString("url");

               // timeInMs = Long.parseLong(time);
                info e1 = new info(mag,location,time,details);
